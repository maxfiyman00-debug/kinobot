...
from keyboards import user_main_kb, subscribe_kb, movie_inline_kb
...
async def check_subscription(bot: Bot, user_id: int):
    ...
    for ch in channels:
        try:
            ...
            # Xatolik yuz bersa adminga xabar yuboramiz
            try:
...
async def start_cmd(message: Message, command: CommandObject, bot: Bot):
    ...
    if movie_code:
        ...
        await message.answer("Xush kelibsiz! Kinolarni qidirish uchun kodni yuboring yoki quyidagi tugmadan foydalaning.", reply_markup=user_main_kb())
...
async def search_movie(message: Message, bot: Bot):
    ...
    # Check if text is just a number (movie code)
